// Persistence for a user's Google Drive connection. The refresh token (and any
// cached access token) is AES-GCM encrypted with the same key material as
// YouTube tokens before it hits Postgres — the DB never holds a usable
// credential. Only this module reads/writes the ciphertext.

import { decryptSecret, encryptSecret } from "@/lib/crypto";
import { createServiceClient } from "@/lib/supabase/server";
import { refreshAccessToken, type GoogleTokens } from "./oauth";
import type { DriveAccount } from "./client";

// Kept deliberately structural (mirrors the remote `drive_connections` table) —
...
export interface DriveConnection extends DriveConnectionRow {
...
export async function saveConnection(
  ...
  const now = new Date();
  const row: Partial<DriveConnectionRow> = {
    user_id: userId,
    drive_email: account.email,
    drive_name: account.name,
  ...
  const { error } = await db().from("drive_connections").upsert(row, { onConflict: "user_id" });
  if (error) throw new Error(`Could not save Google Drive connection: ${error.message}`);
}
...
async function applyTokens(
  ...
  const update: Partial<DriveConnectionRow> = {
  ...
  const { error } = await db().from("drive_connections").update(update).eq("user_id", userId);
  if (error) throw new Error(`Could not cache Google Drive access token: ${error.message}`);
}
...
export async function deleteConnection(userId: string): Promise<void> {
  const { error } = await db().from("drive_connections").delete().eq("user_id", userId);
  if (error) throw new Error(`Could not remove Google Drive connection: ${error.message}`);
}